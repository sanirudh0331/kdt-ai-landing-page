...
// FDA Activity cache (approvals, CRLs, recalls)
let fdaActivityCache = {
    approvals: { data: [], timestamp: 0 },
    crl: { data: [], timestamp: 0 },
    recalls: { data: [], timestamp: 0 }
};
const FDA_ACTIVITY_CACHE_DURATION = 24 * 60 * 60 * 1000; // 24 hours

// Fetch recent FDA drug approvals
async function fetchFDAApprovals() {
    try {
        // Get approvals from last 60 days
        const sixtyDaysAgo = new Date();
        sixtyDaysAgo.setDate(sixtyDaysAgo.getDate() - 60);
        const fromDate = sixtyDaysAgo.toISOString().split('T')[0].replace(/-/g, '');

        const url = `https://api.fda.gov/drug/drugsfda.json?search=submissions.submission_status_date:[${fromDate}+TO+${new Date().toISOString().split('T')[0].replace(/-/g, '')}]+AND+submissions.submission_status:"AP"&limit=20`;

        const response = await fetch(url);
        if (!response.ok) throw new Error(`FDA API error: ${response.status}`);

        const data = await response.json();
        const approvals = (data.results || []).map(drug => {
            const submission = drug.submissions?.find(s => s.submission_status === 'AP') || drug.submissions?.[0] || {};
            return {
                type: 'approval',
                brand_name: drug.products?.[0]?.brand_name || drug.sponsor_name || 'Unknown',
                generic_name: drug.products?.[0]?.active_ingredients?.[0]?.name || '',
                sponsor: drug.sponsor_name || 'Unknown',
                date: submission.submission_status_date || '',
                application_number: drug.application_number || '',
                url: `https://www.accessdata.fda.gov/scripts/cder/daf/index.cfm?event=overview.process&ApplNo=${(drug.application_number || '').replace(/\D/g, '')}`
            };
        }).filter(a => a.date);

        return approvals.sort((a, b) => b.date.localeCompare(a.date));
    } catch (error) {
        console.error('Error fetching FDA approvals:', error.message);
        return [];
    }
}

// Fetch Complete Response Letters
async function fetchFDACRLs() {
    try {
        const url = 'https://api.fda.gov/transparency/crl.json?limit=20&sort=letter_issue_date:desc';

        const response = await fetch(url);
        if (!response.ok) throw new Error(`FDA CRL API error: ${response.status}`);

        const data = await response.json();
        const crls = (data.results || []).map(crl => ({
            type: 'crl',
            drug_name: crl.product_name || 'Unknown',
            sponsor: crl.sponsor || 'Unknown',
            date: crl.letter_issue_date || '',
        ...
        console.error('Error fetching FDA CRLs:', error.message);
        return [];
    }
}

// Fetch drug recalls/enforcement
async function fetchFDARecalls() {
    try {
        const url = 'https://api.fda.gov/drug/enforcement.json?search=report_date:[NOW-60DAYS+TO+NOW]&limit=20&sort=report_date:desc';

        const response = await fetch(url);
        if (!response.ok) throw new Error(`FDA Recalls API error: ${response.status}`);

        const data = await response.json();
        const recalls = (data.results || []).map(recall => ({
            type: 'recall',
            product: recall.product_description?.substring(0, 100) || 'Unknown',
            company: recall.recalling_firm || 'Unknown',
            date: recall.report_date || '',
            reason: recall.reason_for_recall?.substring(0, 150) || '',
        ...
        console.error('Error fetching FDA recalls:', error.message);
        return [];
    }
}

// FDA Activity endpoint
app.get('/api/fda-activity', async (req, res) => {
    const filter = req.query.filter || 'all';
    const now = Date.now();

    try {
        // Refresh caches if stale
        if (now - fdaActivityCache.approvals.timestamp > FDA_ACTIVITY_CACHE_DURATION) {
            fdaActivityCache.approvals.data = await fetchFDAApprovals();
            fdaActivityCache.approvals.timestamp = now;
        }
        if (now - fdaActivityCache.crl.timestamp > FDA_ACTIVITY_CACHE_DURATION) {
            fdaActivityCache.crl.data = await fetchFDACRLs();
            fdaActivityCache.crl.timestamp = now;
        }
        if (now - fdaActivityCache.recalls.timestamp > FDA_ACTIVITY_CACHE_DURATION) {
            fdaActivityCache.recalls.data = await fetchFDARecalls();
            fdaActivityCache.recalls.timestamp = now;
        }

        let events = [];
        if (filter === 'all' || filter === 'approvals') {
            events = events.concat(fdaActivityCache.approvals.data);
        }
        if (filter === 'all' || filter === 'crl') {
            events = events.concat(fdaActivityCache.crl.data);
        }
        if (filter === 'all' || filter === 'recalls') {
            events = events.concat(fdaActivityCache.recalls.data);
        }
        ...
        console.error('FDA Activity error:', error);
